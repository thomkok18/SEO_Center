<x-app-layout
    :active-page="'admin_import_fields_links'"
    :css="asset('css/admin/links/import-fields.min.css')"
    :font-awesome="true"
>
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.links.process-import') }}" method="POST">
                        @csrf
                                        ...
                                        <td>
                                            <select class="form-control" name="fields[{{ $key }}]" title="{{__('title.database_column')}}">
                                                @foreach($document_fields as $field)
                                                    <option value="{{ $field }}"
                                                            @if($key === $loop->index) selected @endif>{{ __('link.' . $field) }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                    @endforeach
